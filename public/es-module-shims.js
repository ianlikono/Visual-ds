...
(function () {
  ...
  function parse(Q, B = "@") {
    ...
    const I = [],
      g = [];
    for (; A.ri(); )
      I.push({ s: A.is(), e: A.ie(), ss: A.ss(), se: A.se(), d: A.id() });
      ...
      if (!source) {
        try {
          ...
          const regexExp = new RegExp(
            "application/(json|javascript|jose+jwe)(;(( q=(1|0.d))|( charset.utf8))+)?,?"
          );
          if (contentType.match(regexExp)) {
    ...
    load.L = load.f.then(async (deps) => {
      load.d = await Promise.all(
        deps.map(async (depId) => {
          ...
          if (importShim$1.skip.test(resolved)) return { b: resolved };
          const depAlias = resolved.startsWith("data:")
            ? new URL(depId, load.alias).href
            : resolved;
        ...
        importMapPromise = importMapPromise.then(
          async () =>
            (importMap = resolveAndComposeImportMap(
              script.src
                ? await (await fetch(script.src)).json()
